...
from .status_page import REPOS_DATA_NAMES as CANDIDATES
 ...
 * target refers to the os_code_name + the arch
...
def _get_blacklist(build_file):
    ...
    Returns a dictionary mapping the package name to a set of tuples of (os_name, os_code_name, arch) that are blacklisted
    """
...
def get_status_description(build_status, expected, blacklist, candidates=CANDIDATES, skip_source=False, debug=False):
    """
    build_status is a recursive structure that maps a specific build i.e. (os_name, os_code_name, arch, candidate)
    to a version number.
    ...
    The blacklist parameter is a set of (os_name, os_code_name, arch) that we don't expect to see (specific to this package)
    """

    # The first thing we do is reverse the mapping
    # version_map maps version to a list of (os_name, os_code_name, arch, candidate)
        ...
        for os_code_name in expected[os_name]:
            ...
            for arch in expected[os_name][os_code_name]:
                ...
                for candidate in candidates:
                    version = None
                    if candidate in arch_d:
                        version = VERSION_PATTERN.match(arch_d[candidate]).group(1)
                    ...
                    version_map[version].append((os_name, os_code_name, arch, candidate))
    ...
    if len(version_map) == 2:
        if candidate_map[None] == set(['main']):
        ...
        elif set(['main']) in candidate_map.values():
    ...
    # If there are three different versions availble
        ...
        if candidate_map[None] == set(['main']) and \
    ...
    if None in version_map:
        ...
        broken_set = set([(os_code_name, arch) for os_name, os_code_name, arch, candidate in version_map[None]])
        if len(broken_set) == 1:
    ...
    for name, m in [('os', os_map), ('code_name', code_name_map), ('arch', arch_map), ('candidate', candidate_map),
                    ('target', target_map)]:
        print(name)
