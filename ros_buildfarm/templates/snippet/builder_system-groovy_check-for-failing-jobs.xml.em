...
boolean are_previous_builds_failing(int number_of_builds_failing,
                                    Job job)
{
  result = true
  ...
  ((last_run_number -1)..(last_run_number - (min_number_of_failures-1))).each { num ->
    ...
    if (job.getBuildByNumber(num) && job.getBuildByNumber(num).result != hudson.model.Result.FAILURE)
    {
      result = false
      return
    }
