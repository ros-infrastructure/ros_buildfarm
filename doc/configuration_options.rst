...
* ``publish_test_reports``: a boolean flag (default: ``False``) controlling
  whether test reports generated by test packages in devel jobs should be
  uploaded to the ``repo`` host.
  Enabling this will copy the contents of the ``test_results`` folder
  to the ``repo`` host.
  A special macro in the ROS wiki will then display links to these reports
  as part of the auto-generated *Package Header*.
